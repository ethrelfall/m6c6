...
# ndot = - (nu)' + (Sn = nstar)
# udot = - u u' - tau T n'/n ELECTRON VELOCITY
...
T0 = 1.0  # temperature
bohm_fac = sqrt(tau)  # electron outflow speed is \sqrt{T} times this factor TRIALCODE - MUST NOT BE GREATER THAN \sqrt{tau*Temp}
#bohm_fac = exp(3.0)  # determines electron outflow speed, 3.0 is the Coulomb logarithm
...
#File("RR_LAPD_1D_two_fluid_init.pvd").write(nuv.sub(0), nuv.sub(1), nuv.sub(2))
#quit()
...
# TODO look at effect of different outflow velocity on weak bdy term in v
...
# last terms above: trying weak outflow BCs as the Dirichlet ones don't work for bohm_fac != 1.0
...
# are they needed?  weak term in above ...
...
outfile = File("RR_LAPD_1D_two_fluid.pvd")
...
alpha_u = sqrt(Temp)*bohm_fac2*exp(-0.5*bohm_fac2*bohm_fac2*Temp*Temp/(tau*Temp))/(nstar)  # constant of integration, fixed by source amp and sonic outflow BCs
...
beta_div_nstar = 1.0/(sqrt(Temp)*bohm_fac)*math.exp(0.5)

for i in range(0,(meshres+1)):
   if mesh.coordinates.dat.data[i] < 0.0:
      u_analytic.dat.data[i] = -sqrt(-tau*T0*lambertw(-mesh.coordinates.dat.data[i]*mesh.coordinates.dat.data[i]*alpha_u*alpha_u*(nstar*nstar)/(tau*T0),0)).real
      tempval = -sqrt(-tau*T0*lambertw(-mesh.coordinates.dat.data[i]*mesh.coordinates.dat.data[i]*alpha_u*alpha_u*(nstar*nstar)/(tau*T0),0)).real
...
File("RR_LAPD_1D_two_fluid_final.pvd").write(ns, us, vs, n_analytic, u_analytic, v_analytic)
